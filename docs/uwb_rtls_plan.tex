...
\textbf{Option A (safe):} Reduce \texttt{UWB\_REPLY\_DELAY\_US} 7000 $\to$ 5500\,µs. Verify exchanges still complete.

\textbf{Option B (aggressive):} Switch to \texttt{MODE\_SHORTDATA\_FAST\_LOWPOWER} (6.8\,Mbps, 64-symbol preamble). Reply delay drops to $\sim$1500\,µs giving 8--10\,Hz for 3 anchors. Cost: $\sim$2\,m range reduction (acceptable for $\leq$15\,m indoor use).
...
\midrule
1.0 & Switch to 64\,MHz PRF (\texttt{MODE\_LONGDATA\_RANGE\_ACCURACY}) & \easy & Pending \\
1.1 & Better calibration (200 samples, multi-distance) & \easy & Pending \\
1.2 & Adaptive polling --- skip dead anchors (firmware) & \easy & Partial \\
1.3 & Faster sweep rate (reduce reply delay / mode change) & \medium & Pending \\
...
1.0 & $\to$ & 1.1 & $\to$ & 1.2 & $\to$ & 1.5 & $\to$ & 1.3 & $\to$ & 2.1 & $\to$ & 2.2 \\
PRF && Cal && Poll && Survey && Rate && NLOS && Wgt \\[4pt]
