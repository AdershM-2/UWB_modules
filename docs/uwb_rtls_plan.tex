...
\begin{donebox}
\textbf{Status (2026-06-11):} \done{} --- No extra sketch or reflash needed. Two new UWB message types (\texttt{MSG\_SURVEY\_REQ}/\texttt{MSG\_SURVEY\_RESP}) added to \texttt{UwbFrame} and \texttt{TwrEngine}. Tag accepts \texttt{SURVEY\textbackslash n} over Serial: loops over all anchor pairs, sends \texttt{MSG\_SURVEY\_REQ} to Anchor\,A (which calls \texttt{rangeTo(B)} and replies), averages 100 samples per pair ($\approx$4\,s/pair). \texttt{matlab/runSurvey.m} collects results, runs classical MDS, fixes the coordinate frame, and writes \texttt{config/anchors.json}. Run \texttt{run\_localization.m} immediately after --- no reflash required.
\end{donebox}
...
1.5 & Anchor self-survey + auto \texttt{anchors.json} & \medium & \done \\
\midrule
